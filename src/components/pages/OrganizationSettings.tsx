    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-slate-600">Loading organization settings...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="flex items-center justify-between py-6">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-slate-600">{organization?.name} - {organization?.subdomain}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
          ...
          {members.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-slate-500 mb-4">Add members to collaborate in this organization</p>
              {canManageMembers && (
                <button
                  onClick={() => setShowAddMemberForm(true)}
                ...
                <div key={member.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-lg">
                  ...
                  {canManageMembers && member.id !== user?.id && (
                    <div className="flex items-center space-x-2">
                      <select
                        value={member.role}
                        onChange={(e) => handleUpdateMemberRole(member.id, e.target.value as 'OWNER' | 'ADMIN' | 'USER')}
        ...
        {showAddMemberForm && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md">
              <h3 className="text-lg font-semibold mb-4">Add Organization Member</h3>
              ...
              <form onSubmit={handleAddMember}>
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <select
                      value={newMemberRole}
                      onChange={(e) => setNewMemberRole(e.target.value as 'OWNER' | 'ADMIN' | 'USER')}
                      className="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-primary-500 focus:border-primary-500"
                    >
                ...
                <div className="flex justify-end space-x-3 mt-6">
                  ...
                  <button
                    type="submit"
                    disabled={adding}
                    className="px-4 py-2 text-sm font-medium text-white bg-amber-800 rounded-md hover:bg-amber-800 disabled:opacity-50"
                  >
